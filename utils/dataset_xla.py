...
def load_chunk(np_rng: np.random.Generator, path, length, chunk_size, offset=None):
    if chunk_size <= length:
        if offset is None:
            offset = np_rng.integers(length - chunk_size + 1)
...
class MSSDataset(torch.utils.data.IterableDataset):
    def __init__(self, config, data_path, metadata_path="metadata.pkl", dataset_type=1, verbose=True,
                 base_seed=42, samples_per_worker = 1000,  input_time_domain = True, output_time_domain = True):
    ...
    def init_random_generator(self) -> (random.Random, np.random.Generator):
        ...
        # print(f'init random generator for global_worker_id {global_worker_id} ({total_workers} workers)')


        seed = self.base_seed + self.epoch * 100000 + global_worker_id
        # rng = torch.Generator()
    ...
    def read_from_metadata_cache(self, track_paths, instr=None):
        ...
        if os.path.isfile(self.metadata_path):
            if self.verbose:
                xm.master_print('Found metadata cache file: {}'.format(self.metadata_path))
        ...
        if len(metadata) > 0:
            xm.master_print('Old metadata was used for {} tracks.'.format(len(metadata)))
        return track_paths, metadata
    ...
    def get_metadata(self):
        ...
        if self.dataset_type in [1, 4]:
            ...
            if type(self.data_path) == list:
                for tp in self.data_path:
                    ...
                    if len(tracks_for_folder) == 0:
                        xm.master_print('Warning: no tracks found in folder \'{}\'. Please check it!'.format(tp))
            ...
            for i in range(len(self.data_path)):
                if self.verbose:
                    xm.master_print('Reading tracks from: {}'.format(self.data_path[i]))
                df = pd.read_csv(self.data_path[i])

                skipped = 0
                for instr in self.instruments:
                    part = df[df['instrum'] == instr].copy()
                    xm.master_print('Tracks found for {}: {}'.format(instr, len(part)))
                for instr in self.instruments:
                    part = df[df['instrum'] == instr].copy()
                    metadata[instr] = []
                    ...
                    for path in tqdm(track_paths):
                        if not os.path.isfile(path):
                            xm.master_print('Cant find track: {}'.format(path))
                            skipped += 1
                            continue
                        # xm.master_print(path)
                        try:
                            length = len(sf.read(path)[0])
                        except:
                            xm.master_print('Problem with path: {}'.format(path))
                            skipped += 1
                            continue
                        metadata[instr].append((path, length))
                if skipped > 0:
                    xm.master_print('Missing tracks: {} from {}'.format(skipped, len(df)))
        else:
            xm.master_print('Unknown dataset type: {}. Must be 1, 2, 3 or 4'.format(self.dataset_type))
            exit()
    ...
    def load_source(self, rng: random.Random, np_rng: np.random.Generator, metadata, instr):
        while True:
            if self.dataset_type in [1, 4]:
                track_path, track_length = rng.choice(metadata)
                for extension in self.file_types:
                    path_to_audio_file = track_path + '/{}.{}'.format(instr, extension)
                    if os.path.isfile(path_to_audio_file):
                        try:
                            source = load_chunk(np_rng, path_to_audio_file, track_length, self.chunk_size)
                        except Exception as e:
                            # Sometimes error during FLAC reading, catch it and use zero stem
                            xm.master_print('Error: {} Path: {}'.format(e, path_to_audio_file))
                            source = np.zeros((2, self.chunk_size), dtype=np.float32)
                        break
            else:
                track_path, track_length = rng.choice(metadata[instr])
                try:
                    source = load_chunk(np_rng, track_path, track_length, self.chunk_size)
                except Exception as e:
                    # Sometimes error during FLAC reading, catch it and use zero stem
                    xm.master_print('Error: {} Path: {}'.format(e, track_path))
                    source = np.zeros((2, self.chunk_size), dtype=np.float32)

            if np.abs(source).mean() >= self.min_mean_abs:  # remove quiet chunks
                break
        if self.aug:
            source = self.augm_data(rng, source, instr)
        return torch.tensor(source, dtype=torch.float32)

    def load_random_mix(self, rng: random.Random, np_rng: np.random.Generator):
        res = []
        for instr in self.instruments:
            s1 = self.load_source(rng, np_rng, self.metadata, instr)
                    ...
                    if self.config['augmentations'].mixup:
                        ...
                        for prob in self.config.augmentations.mixup_probs:
                            if rng.uniform(0, 1) < prob:
                                s2 = self.load_source(rng, np_rng, self.metadata, instr)
                                mixup.append(s2)
                        mixup = torch.stack(mixup, dim=0)
                        loud_values = np_rng.uniform(
    ...
    def load_aligned_data(self, rng: random.Random, np_rng: np.random.Generator):
        track_path, track_length = rng.choice(self.metadata)
        attempts = 10
        while attempts:
            if track_length >= self.chunk_size:
                common_offset = np_rng.integers(track_length - self.chunk_size + 1)
            else:
                common_offset = None
            res = []
            silent_chunks = 0
            ...
            attempts -= 1
            if attempts <= 0:
                xm.master_print('Attempts max!', track_path)
        ...
        if self.aug:
            for i, instr in enumerate(self.instruments):
                res[i] = self.augm_data(rng, res[i], instr)
        return torch.tensor(res, dtype=torch.float32)

    def augm_data(self, rng: random.Random, source, instr):
        ...
        if 'stereo_to_mono' in augs:
            if augs['stereo_to_mono'] > 0:
                if rng.uniform(0, 1) < augs['stereo_to_mono']:
                    # Generate random perceptual weights that sum to 1
                    w_left = rng.uniform(0.0, 1.0)
                    w_right = 1.0 - w_left
        ...
        if 'pitch_shift' in augs:
            if augs['pitch_shift'] > 0:
                if rng.uniform(0, 1) < augs['pitch_shift']:
        ...
        if 'seven_band_parametric_eq' in augs:
            if augs['seven_band_parametric_eq'] > 0:
                if rng.uniform(0, 1) < augs['seven_band_parametric_eq']:
        ...
        if 'tanh_distortion' in augs:
            if augs['tanh_distortion'] > 0:
                if rng.uniform(0, 1) < augs['tanh_distortion']:
        ...
        if 'mp3_compression' in augs:
            if augs['mp3_compression'] > 0:
                if rng.uniform(0, 1) < augs['mp3_compression']:
        ...
        if 'gaussian_noise' in augs:
            if augs['gaussian_noise'] > 0:
                if rng.uniform(0, 1) < augs['gaussian_noise']:
        ...
        if 'time_stretch' in augs:
            if augs['time_stretch'] > 0:
                if rng.uniform(0, 1) < augs['time_stretch']:
        ...
        if 'pedalboard_reverb' in augs:
            if augs['pedalboard_reverb'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_reverb']:
        ...
        if 'pedalboard_chorus' in augs:
            if augs['pedalboard_chorus'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_chorus']:
                    rate_hz = rng.uniform(
        ...
        if 'pedalboard_phazer' in augs:
            if augs['pedalboard_phazer'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_phazer']:
                    rate_hz = rng.uniform(
                    ...
                    centre_frequency_hz = rng.uniform(
        ...
        if 'pedalboard_distortion' in augs:
            if augs['pedalboard_distortion'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_distortion']:
                    drive_db = rng.uniform(
        ...
        if 'pedalboard_pitch_shift' in augs:
            if augs['pedalboard_pitch_shift'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_pitch_shift']:
        ...
        if 'pedalboard_resample' in augs:
            if augs['pedalboard_resample'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_resample']:
                    target_sample_rate = rng.uniform(
        ...
        if 'pedalboard_bitcrash' in augs:
            if augs['pedalboard_bitcrash'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_bitcrash']:
                    bit_depth = rng.uniform(
        ...
        if 'pedalboard_mp3_compressor' in augs:
            if augs['pedalboard_mp3_compressor'] > 0:
                if rng.uniform(0, 1) < augs['pedalboard_mp3_compressor']:
                    vbr_quality = rng.uniform(
        ...
        # xm.master_print(applied_augs)
        return source

    def __iter__(self):
        rng, np_rng = self.init_random_generator()
        # infinite generation
        for idx in range(self.samples_per_worker):
            if self.dataset_type in [1, 2, 3]:
                res = self.load_random_mix(rng, np_rng)
            else:
                res = self.load_aligned_data(rng, np_rng)
